...
const LoginView = ({
  ...
  // ⚡ NOUVEAU : L'état qui gère l'animation d'attente vers le Lobby
  const [isConnecting, setIsConnecting] = useState(false);

  useEffect(() => {
    let loginMusic = null;
    if (hasEntered) {
      loginMusic = playCrossfadeLoop("loginloop.mp3", 0.3, 2000);
    }
    return () => {
  ...
  // ⚡ Fonction pour intercepter le clic, animer, et appeler la Base de données
  const handleAction = async (actionCallback) => {
    playSound("wood-button.mp3", 0.4, null);
    setIsConnecting(true); // Déclenche l'animation instantanément !

    try {
      await actionCallback(); // Appelle Supabase
    } catch (error) {
      console.error(error);
    } finally {
      // Si la requête échoue (ex: faux code), on remet le formulaire au bout de 2s
  ...
  return (
    <div className="relative min-h-screen flex flex-col items-center justify-center p-4 bg-[url('/background.png')] bg-cover bg-center bg-no-repeat text-amber-50 overflow-hidden">
      ...
      {/* ========================================== */}
      {/* L'INTERFACE PRINCIPALE ET L'ANIMATION */}
      {/* ========================================== */}
      <div
      ...
      >
        ...
        {/* Le conteneur parent (qui garde la taille fixe pour que rien ne saute) */}
        <div className="relative w-full max-w-sm flex justify-center">
          {/* 1. LE FORMULAIRE (S'efface quand isConnecting est vrai) */}
          <div
          ...
          {/* 2. L'ANIMATION D'ATTENTE (Apparaît quand isConnecting est vrai) */}
          <div
